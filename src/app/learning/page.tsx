  ...
  return (
    <div className="page-pad" style={{padding:"28px 32px",background:"var(--bg)",minHeight:"100%"}}>
      <div className="fu" style={{marginBottom:24}}>
        <h1 style={{fontSize:24,fontWeight:900,color:"var(--text)",letterSpacing:"-.5px"}}>Learning Plan Generator</h1>
        <p style={{fontSize:13,color:"var(--muted)",marginTop:6}}>Paste a syllabus or upload a PDF → get a <span className="grad-text" style={{fontWeight:700}}>24-week structured roadmap</span></p>
      </div>
      ...
      {!plan ? (
        <div style={{maxWidth:680}}>
          ...
          <button onClick={generate} disabled={loading||(mode==="text"?text.trim().length<20:!file)} className="btn-glow" style={{...btn(true),marginTop:14,width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:8,opacity:loading?.6:1,fontSize:15,letterSpacing:".2px"}}>
        ...
        <div className="fu">
          ...
          <div style={{marginBottom:18}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
              <span style={{fontSize:11,color:"var(--muted)",fontWeight:600}}>Plan Progress</span>
              <span style={{fontSize:11,color:"var(--accent)",fontFamily:"monospace"}}>{plan.weeks?.length || 0}/24 weeks</span>
            </div>
            <div style={{height:6,background:"var(--surface2)",borderRadius:99,overflow:"hidden"}}>
              <div style={{height:"100%",width:`${((plan.weeks?.length || 0)/24)*100}%`,background:"linear-gradient(90deg,#4f8ef7,#a78bfa,#34d399)",borderRadius:99,transition:"width .8s cubic-bezier(.22,1,.36,1)"}} />
            </div>
          </div>
          ...
          <div style={{display:"flex",gap:8,marginBottom:12}}>
            <button onClick={()=>setOpen(new Set(plan.weeks?.map(w=>w.week)||[]))} style={{padding:"5px 14px",borderRadius:8,border:"1px solid var(--border)",background:"var(--surface2)",color:"var(--muted)",fontSize:11,cursor:"pointer",fontFamily:"inherit"}}>Expand All</button>
            <button onClick={()=>setOpen(new Set())} style={{padding:"5px 14px",borderRadius:8,border:"1px solid var(--border)",background:"var(--surface2)",color:"var(--muted)",fontSize:11,cursor:"pointer",fontFamily:"inherit"}}>Collapse All</button>
          </div>
