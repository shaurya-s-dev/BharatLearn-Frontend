  ...
  return (
    <div className="page-pad" style={{padding:"28px 32px",background:"var(--bg)",minHeight:"100%"}}>
      <div className="fu" style={{marginBottom:24}}>
        <h1 style={{fontSize:24,fontWeight:900,color:"var(--text)",letterSpacing:"-.5px"}}>Quiz Generator</h1>
        <p style={{fontSize:13,color:"var(--muted)",marginTop:6}}>Pick a topic → get 10 AI questions: <span style={{color:"#a78bfa"}}>MCQ</span> · <span style={{color:"#4f8ef7"}}>Short Answer</span> · <span style={{color:"#f87171"}}>Coding</span></p>
      </div>
      ...
      {!quiz ? (
        <div style={{maxWidth:660}}>
          ...
          <button onClick={gen} disabled={!topic.trim()||loading}
            className="btn-glow"
            style={{width:"100%",padding:"14px",borderRadius:12,border:"none",cursor:"pointer",background:"linear-gradient(135deg,#a78bfa,#4f8ef7)",color:"#fff",fontSize:15,fontWeight:700,opacity:!topic.trim()||loading?.5:1,display:"flex",alignItems:"center",justifyContent:"center",gap:8,letterSpacing:".2px"}}>
        ...
        <div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            {filtered.map((q,idx)=>(
              <div key={q.id} style={{animation:`fadeUp .35s ${idx*0.04}s cubic-bezier(.22,1,.36,1) both`}}><QCard q={q}
