  ...
  return (
    <div
      ...
      style={{
        ...
        background: flash ? `${clr}10` : hovered ? `${clr}06` : "var(--surface)",
        border: `1px solid ${flash ? clr + "40" : hovered ? clr + "20" : "var(--border)"}`,
        transition:"all .4s ease",
        animation: `fadeUp .4s ${delay || 0}s cubic-bezier(.22,1,.36,1) both`,
      }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10 }}>
        <p style={{ fontSize:9, color:"var(--muted)", textTransform:"uppercase", letterSpacing:2, fontWeight:700 }}>{label}</p>
        <span style={{ fontSize:20, transition:"transform .3s", transform: hovered ? "scale(1.2)" : "scale(1)" }}>{icon}</span>
      </div>
      <p style={{ fontSize:32, fontWeight:900, color:clr, lineHeight:1, fontFamily:"monospace", transition:"all .3s" }}>{val}</p>
      <p style={{ fontSize:11, color:"var(--muted)", marginTop:8 }}>{sub}</p>
      <div style={{ height:4, background:"var(--surface2)", borderRadius:99, marginTop:14, overflow:"hidden" }}>
        <div style={{ height:"100%", width:`${pct}%`, background:`linear-gradient(90deg, ${clr}, ${clr}aa)`, borderRadius:99, transition:"width 0.8s cubic-bezier(.22,1,.36,1)" }}/>
      </div>
    </div>
  );
}
...
export default function Dashboard() {
  ...
  return (
    <div className="page-pad" style={{ padding:"28px 32px", background:"var(--bg)", minHeight:"100%" }}>
      ...
      {/* Stats */}
      <div className="grid-stats" style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:22 }}>
        {stats.map((s,i) => <StatCard key={s.label} {...s} flash={flashStats} delay={i * 0.06} />)}
      </div>

      {/* Charts */}
      <div className="grid-charts" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:18 }}>
        <div style={card}>
      ...
      <div className="card-hover" style={card}>
        ...
        <div className="grid-stats" style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10 }}>
          {[
            ...
            <a key={a.href} href={a.href} className="btn-glow" style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:8, padding:"18px 12px", borderRadius:14, textDecoration:"none", background:`${a.clr}10`, border:`1px solid ${a.clr}25`, transition:"all .2s" }}>
              <span style={{ fontSize:24, color:a.clr }}>{a.icon}</span>
              <span style={{ fontSize:12, fontWeight:600, color:"var(--text)", textAlign:"center" }}>{a.label}</span>
            </a>
          ))}
        </div>
      </div>
